...
* **How to use:** Happens automatically if the LLM is loaded. In order to rate images, load a vision-enabled model, like Gemma 3 or GPT 4.1. AI models can be loaded locally using LM Studio or in the cloud using the OpenAI API (requires key). Once the LLM's job is done, the ratings will be used to improve subsequent results even if the LLM is not loaded.
